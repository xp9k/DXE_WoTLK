...
--[[	
	if srcName and destName and spellName then
		print(srcName.. "->" .. destName.. "<" .. spellName..">"..OUTPUT)
	end
]]--
	...
	if not (UnitInRaid(srcName) or UnitInParty(srcName))then return end -- If the caster isn't in the raid group	
	...
	if event == "SPELL_CAST_SUCCESS" then
		...
		elseif rituals[spellID] then
			send(create:format(icon(srcName), srcName, GetSpellLink(spellID))) -- [X] is casting a [Z] -- Rituals
		elseif misc[spellID] then
			send(miscellaneous:format(srcName, GetSpellLink(spellID))) --Misc
		end
